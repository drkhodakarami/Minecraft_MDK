package jiraiyah.modmdk.utils.fluid;

import com.google.common.base.Preconditions;
import com.mojang.blaze3d.systems.RenderSystem;
import jiraiyah.modmdk.ModMain;
import jiraiyah.modmdk.utils.interfaces.IIngredientRenderer;
...
public class FluidStackRenderer implements IIngredientRenderer<FluidStack>
{
    ...
    public List<Text> getTooltip(FluidStack fluidStack, TooltipContext tooltipFlag)
    {
        ...
        {
            MutableText amountString = Text.translatable(ModMain.ModID + ".tooltip.liquid.amount.with.capacity", nf.format(amount), nf.format(capacityMb));
            tooltip.add(amountString.fillStyle(Style.EMPTY.withColor(Formatting.DARK_GRAY)));
        }
        else if (tooltipMode == TooltipMode.SHOW_AMOUNT)
        {
            MutableText amountString = Text.translatable(ModMain.ModID + ".tooltip.liquid.amount", nf.format(amount));
            tooltip.add(amountString.fillStyle(Style.EMPTY.withColor(Formatting.DARK_GRAY)));
        }
